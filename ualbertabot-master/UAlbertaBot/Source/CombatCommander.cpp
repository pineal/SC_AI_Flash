...
void CombatCommander::updateAttackSquads()
{
	...
	if (UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Dragoon) < UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Zealot)*4)
	{
		mainAttackSquad._microDecision.setSaveZealotLess(25);
	}
	else
	{
		mainAttackSquad._microDecision.setSaveZealotLess(0);
	}
	mainAttackSquad._microDecision.setFlightOrientatedFactor(std::min(1+UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Carrier)*0.5,3.0));
    for (auto & unit : _combatUnits)
    {
...
void CombatCommander::updateDefenseSquads() 
{
	...
	for (BWTA::Region * myRegion : InformationManager::Instance().getOccupiedRegions(BWAPI::Broodwar->self()))
	{
		...
		BWAPI::Unitset enemyUnitsInRegion;
		int groundCombatNum = 0,flyerCombatNum= 0;
        ...
        if (_squadData.squadExists(squadName.str()))
        {
			...
			defenseSquad._microDecision.setSaveZealotLess(0);
			defenseSquad._microDecision.setFlightOrientatedFactor(std::min(1 + UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Carrier)*0.5, 3.0));
            // figure out how many units we need on defense
			int flyingDefendersNeeded = numEnemyFlyingInRegion * 2;
			int groundDefensersNeeded = numEnemyGroundInRegion * 2;
...
BWAPI::Unit CombatCommander::findClosestDefender(const Squad & defenseSquad, BWAPI::Position pos, bool flyingDefender) 
{
	...
	for (auto & unit : _combatUnits) 
	{
		...
		// dist increase 2000 for workers, and don't choose worker after 10min
		int minute = BWAPI::Broodwar->getFrameCount() / 24 / 60;
		int dist = unit->getDistance(pos) + unit->getType().isWorker() ? 2000 : 0;
		if (unit->isCarryingGas())
			dist -= 500;
		//                                      Do not choose workers from other bases                         Do not choose workers after 10 minutes
		if (!(unit->getType().isWorker() && BWTA::getRegion(pos) != BWTA::getRegion(unit->getPosition())) && !(unit->getType().isWorker() && minute >= 5) && (!closestDefender || dist<minDistance))
		{
