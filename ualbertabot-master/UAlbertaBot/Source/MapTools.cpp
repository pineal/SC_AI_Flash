...
void MapTools::MapGraphOutput(std::ostream &fout)
{/*
auto startRegion = BWAPI::Broodwar->getRegionAt(BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()));
std::unordered_set<int> RegionDup;
std::queue<BWAPI::RegionInterface*> RegionBFS;
RegionBFS.push(startRegion);
RegionDup.insert(startRegion->getID());
while (!RegionBFS.empty())
{
auto now = RegionBFS.front();
fout << now->getCenter().x << " " << now->getCenter().y<<" ";
fout << BWTA::getGroundDistance(BWAPI::TilePosition(now->getCenter()), BWAPI::TilePosition(startRegion->getCenter())) << std::endl;
fout.flush();
for (auto neighbor : now->getNeighbors())
{
if (RegionDup.find(neighbor->getID()) == RegionDup.end())
{
RegionDup.insert(neighbor->getID());
RegionBFS.push(neighbor);
}
}
RegionBFS.pop();
}
...
const std::vector<BWAPI::TilePosition> & MapTools::getClosestTiles(BWAPI::Position pos)
{
