...
void ProductionManager::update() 
{
	...
	if ((_queue.size() == 0) && (BWAPI::Broodwar->getFrameCount() > 10))
	{
		...
		//BWAPI::Broodwar->sendText("%s", "empty queue detected");
...
void ProductionManager::trainInterceptor(){
	int numIncerceptor = BWAPI::Broodwar->self()->completedUnitCount(BWAPI::UnitTypes::Protoss_Interceptor);

	int numCarrier = BWAPI::Broodwar->self()->completedUnitCount(BWAPI::UnitTypes::Protoss_Carrier);


	BWAPI::Unitset Units = BWAPI::Broodwar->self()->getUnits();

	for (auto &u : Units)
		if (u->getType() == BWAPI::UnitTypes::Protoss_Carrier && u->canTrain() && u->isTraining()==false){
			u->train( BWAPI::UnitTypes::Protoss_Interceptor);
		}
}
...
void ProductionManager::manageBuildOrderQueue() 
{
	...
	DecisionMaker::Instance().stat.on_frame();
	...
	while (!_queue.isEmpty()) 
	{
		...
		/*
		if (!_queue.isEmpty() && currentItem.metaType.isBuilding()==false &&
			currentItem.metaType.supplyRequired() + DecisionMaker::Instance().stat.get_population() > DecisionMaker::Instance().stat.get_population_limit()){//cancel units
		...
		*/
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Cybernetics_Core &&
			BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Cybernetics_Core)>0) {//cancel Protoss_Cybernetics_Core
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}

		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Fleet_Beacon &&
			BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Fleet_Beacon)>0) {//cancel Protoss_Fleet_Beacon
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUpgradeType() == BWAPI::UpgradeTypes::Protoss_Air_Weapons &&
			DecisionMaker::Instance().needAirUpgrade()==false) {//cancel air upgrade
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Robotics_Facility && 
			BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Robotics_Facility )>0 ) {//cancel Protoss_Robotics_Facility
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}

		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Observatory &&
			BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Observatory)>0) {//cancel Protoss_Observatory
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Probe && DecisionMaker::Instance().needWorker() == false) {//cancel workers
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Pylon && DecisionMaker::Instance().needSupply() == false && BWAPI::Broodwar->getFrameCount()<= 24*130){ //cancel pylons
			//BWAPI::Broodwar->sendText("%s", "pylons canceled");
			_queue.removeCurrentHighestPriorityItem();
			if (!_queue.isEmpty())
				currentItem = _queue.getHighestPriorityItem();
		}
		...
		// buggy?
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Gateway &&
			DecisionMaker::Instance().needGateway() == false){//cancel gateways
		...
		while (!_queue.isEmpty() && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Stargate &&
			DecisionMaker::Instance().needStargate() == false){//cancel gateways
		...
		if (testflag && currentItem.metaType.getUnitType() == BWAPI::UnitTypes::Protoss_Pylon){
