...
void WorkerData::setWorkerJob(BWAPI::Unit unit, enum WorkerJob job, BWAPI::Unit jobUnit)
{
	...
	if (job == Minerals)
	{
		...
		BWAPI::Unit mineralToMine = getMineralToMine(unit, jobUnit);
...
int WorkerData::getWorkersNearDepot(BWAPI::Unit depot)
{
	...
	{
		if ((unit->getType() == BWAPI::UnitTypes::Protoss_Probe) && unit->getDistance(depot) < 200)
		{
			workersNearDepot++;
		}
	}
...
BWAPI::Unit WorkerData::getMineralToMine(BWAPI::Unit worker, BWAPI::Unit depot)
{
